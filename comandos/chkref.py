...
async def chkref(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        canal_destino = "@vikingchkrefes"  # Canal destino
        ...
        try:
            ...
            if "chat not found" in error_msg:
                await update.message.reply_text(
                    ...
                    "Verifica que el bot tenga acceso al canal: @vikingchkrefes",
                    parse_mode='Markdown'
                )
            elif "bot is not a member" in error_msg:
                await update.message.reply_text(
                    "❌ **Error:** El bot no es miembro del canal.\n"
                    "Asegúrate de que el bot esté agregado como administrador en @vikingchkrefes",
                    parse_mode='Markdown'
                )
            else:
                await update.message.reply_text(
